...
- **UI Integration**: Sidebar navigation fully wired for restaurant roles: `restaurant_manager` sees Restaurant Management + KDS + Waiter View; `kitchen_staff` sees KDS only; `waiter` sees Order Management. Admin/property_manager sidebar includes a Restaurant group with Restaurant Management and KDS links. Finance panel shows a Restaurant Revenue Summary widget (today's revenue, order count, pending settlement). Staff creation modals (admin-dashboard.tsx + owner-dashboard.tsx) now include Restaurant Manager, Waiter, Kitchen Staff roles. Backend role validation (`/api/admin/create-staff`, `/api/properties/:id/staff/create`, invite join) updated to accept the three new restaurant roles. `getRoleDisplayName` in permissions.ts updated.
