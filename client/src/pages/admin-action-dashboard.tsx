...
function CheckInOutPanel() {
  ...
  const checkInMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: t("checkin.success", "Check-in completed") });
    },
  });
  ...
  const checkOutMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/bookings"] });
      toast({ title: t("checkout.success", "Check-out completed") });
    },
  });
    ...
    <div className="p-4 space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
            {t("checkin.todayArrivals", "Today's Arrivals")} · {todayCheckIns.length}
          </p>
        </div>
        {todayCheckIns.length === 0 ? (
          <p className="text-sm text-muted-foreground py-4 text-center">{t("checkin.noArrivals", "No arrivals today")}</p>
        ) : (
          <div className="space-y-2">
            {todayCheckIns.map(b => (
              <motion.div
                ...
                className="flex items-center gap-3 p-3 rounded-xl border border-border/40 bg-card hover:bg-accent/30 transition-colors"
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{b.guestName || t("common.guest", "Guest")}</p>
                ...
                >
                  {checkInMutation.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : t("checkin.checkIn", "Check In")}
                </Button>
              </motion.div>
            ))}
          </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
            {t("checkout.todayDepartures", "Today's Departures")} · {todayCheckOuts.length}
          </p>
        </div>
        {todayCheckOuts.length === 0 ? (
          <p className="text-sm text-muted-foreground py-4 text-center">{t("checkout.noDepartures", "No departures today")}</p>
        ) : (
          <div className="space-y-2">
            {todayCheckOuts.map(b => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{b.guestName || t("common.guest", "Guest")}</p>
                  <p className="text-xs text-muted-foreground">{t("checkout.since", "Since")} {formatDate(b.checkIn)}</p>
                </div>
                <Button
                  size="sm"
                  variant="outline"
                ...
                >
                  {checkOutMutation.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : t("checkout.checkOut", "Check Out")}
                </Button>
              </motion.div>
            ))}
          </div>
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex gap-2">
        {["all", "confirmed", "pending", "checked_in"].map(s => (
          <button
            key={s}
            onClick={() => setFilter(s)}
          ...
          >
            {s === "all" ? t("common.all", "All") : t(`booking.status.${s}`, s)}
          </button>
        ))}
      </div>

      {filtered.length === 0 ? (
        <EmptyState icon={ListChecks} label={t("bookings.empty", "No bookings found")} />
      ) : (
        <div className="space-y-2">
          {filtered.map(b => (
            <motion.div
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {b.totalPrice ? ` · $${(Number(b.totalPrice) / 100).toFixed(0)}` : ""}
                </p>
              </div>
              <Badge className={`text-[10px] px-2 py-0.5 rounded-full ${BOOKING_STATUS_COLORS[b.status] || ""}`}>
...
function RequestsPanel() {
  ...
  const resolveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: t("requests.resolved", "Request resolved") });
    },
  });
    ...
    <div className="p-4 space-y-6">
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
            {t("requests.open", "Open Requests")} · {open.length}
          </p>
        </div>
        {open.length === 0 ? (
          <div className="flex items-center gap-2 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800">
            ...
            <p className="text-sm text-emerald-700 dark:text-emerald-300">{t("requests.allClear", "All requests resolved!")}</p>
          </div>
        ) : (
          <div className="space-y-2">
            {open.map(r => (
              <motion.div
                ...
                initial={{ opacity: 0, y: 4 }}
                animate={{ opacity: 1, y: 0 }}
                className="flex items-start gap-3 p-3 rounded-xl border border-border/40 bg-card hover:bg-accent/30 transition-colors"
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium">{r.requestType || t("requests.service", "Service")}</p>
                  <p className="text-xs text-muted-foreground line-clamp-1">{r.description || "—"}</p>
                  {r.priority && (
                    <Badge className={`mt-1 text-[10px] px-1.5 py-0.5 rounded-full ${REQUEST_PRIORITY_COLORS[r.priority] || ""}`}>
                      {r.priority}
                    </Badge>
                  )}
                </div>
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-7 text-xs px-2 rounded-lg shrink-0 text-emerald-700 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-900/20"
      ...
      {resolved.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">
            {t("requests.recentlyResolved", "Recently Resolved")}
          </p>
          <div className="space-y-2">
            {resolved.map(r => (
              <div key={r.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-muted/40 opacity-60">
                <CheckCircle2 className="h-4 w-4 text-emerald-500 shrink-0" />
                <p className="text-sm truncate">{r.requestType || t("requests.service", "Service")}</p>
              </div>
            ))}
          </div>
        </div>
...
function RoomsPanel() {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: t("roomStatus.updated", "Status updated") });
    },
  });
  ...
  if (!units || units.length === 0) {
    return <EmptyState icon={BedSingle} label={t("rooms.empty", "No rooms found")} />;
  }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          { label: t("roomStatus.ready", "Ready"), count: counts.ready, color: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300" },
          { label: t("roomStatus.occupied", "Busy"), count: counts.occupied, color: "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300" },
          { label: t("roomStatus.dirty", "Dirty"), count: counts.dirty, color: "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300" },
          { label: t("roomStatus.cleaning", "Clean"), count: counts.cleaning, color: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300" },
        ].map(item => (
      ...
      <div className="space-y-2">
        {units.map(unit => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium">{t("rooms.room", "Room")} {unit.unitNumber}</p>
            ...
            >
              <SelectTrigger className={`h-7 text-[11px] w-[100px] rounded-lg border-0 font-medium ${STATUS_COLORS[unit.status] || ""}`}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="available">{t("roomStatus.available", "Available")}</SelectItem>
                <SelectItem value="occupied">{t("roomStatus.occupied", "Occupied")}</SelectItem>
                <SelectItem value="dirty">{t("roomStatus.dirty", "Dirty")}</SelectItem>
                <SelectItem value="cleaning">{t("roomStatus.cleaning", "Cleaning")}</SelectItem>
                <SelectItem value="out_of_order">{t("roomStatus.outOfOrder", "Out of Order")}</SelectItem>
              </SelectContent>
            </Select>
          </motion.div>
        ))}
...
export default function AdminActionDashboard() {
  ...
  const panels: Record<NonNullable<PanelType>, { title: string; subtitle: string; icon: React.ReactNode; content: React.ReactNode }> = {
    checkinout: {
      title: t("admin.checkinout", "Check-in / Check-out"),
      subtitle: `${todayArrivals} ${t("checkin.arrivals", "arrivals")} · ${todayDepartures} ${t("checkout.departures", "departures")}`,
      icon: <ArrowRightLeft className="h-5 w-5 text-primary" />,
      content: <CheckInOutPanel />,
    },
    bookings: {
      title: t("admin.bookings", "Reservations"),
      subtitle: `${(bookings || []).length} ${t("bookings.total", "total")}`,
      icon: <ListChecks className="h-5 w-5 text-primary" />,
      content: <BookingsPanel />,
    },
    requests: {
      title: t("admin.serviceRequests", "Service Requests"),
      subtitle: `${openRequests} ${t("requests.open", "open")}`,
      icon: <BellRing className="h-5 w-5 text-primary" />,
      content: <RequestsPanel />,
    },
    rooms: {
      title: t("admin.roomStatus", "Room Status"),
      subtitle: `${(units || []).length} ${t("rooms.total", "rooms")}`,
  ...
  const actions = [
    {
      ...
      icon: ArrowRightLeft,
      label: t("admin.checkinout", "Check-in / Out"),
      description: checkinBadge > 0 ? `${checkinBadge} ${t("admin.todayAction", "today")}` : t("admin.noAction", "All clear"),
      badge: checkinBadge,
    ...
    },
    {
      ...
      icon: CalendarCheck,
      label: t("admin.bookings", "Reservations"),
      description: `${(bookings || []).length} ${t("bookings.total", "total")}`,
    ...
    },
    {
      ...
      icon: BellRing,
      label: t("admin.serviceRequests", "Service Requests"),
      description: openRequests > 0 ? `${openRequests} ${t("requests.pending", "pending")}` : t("admin.allClear", "All clear"),
      badge: openRequests,
    ...
    },
    {
      ...
      icon: BedDouble,
      label: t("admin.roomStatus", "Room Status"),
      description: dirtyRooms > 0 ? `${dirtyRooms} ${t("rooms.needAttention", "need attention")}` : t("rooms.allReady", "All ready"),
      badge: dirtyRooms,
