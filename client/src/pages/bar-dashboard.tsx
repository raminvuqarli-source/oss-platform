...
import { Wine, Clock, CheckCircle2, RefreshCw, MapPin, BedDouble, GlassWater } from "lucide-react";
...
const STATUS_COLORS: Record<string, string> = {
  pending:   "bg-violet-50 border-violet-300 dark:bg-violet-950 dark:border-violet-700",
  cooking:   "bg-indigo-50 border-indigo-300 dark:bg-indigo-950 dark:border-indigo-700",
...
export default function BarDashboard() {
  ...
  const mixing  = active.filter(o => o.kitchenStatus === "cooking");
  ...
  function OrderCard({ order }: { order: PosOrder }) {
    ...
    return (
      <Card
        ...
        data-testid={`card-bar-order-${order.id}`}
      >
        ...
        <CardHeader className="pb-2 pt-3 px-3 sm:px-6">
          <div className="flex items-start justify-between gap-2">
            ...
            <span className={`text-xs font-bold px-2 py-1 rounded-full shrink-0 ${
              order.kitchenStatus === "pending"   ? "bg-violet-100 text-violet-800 dark:bg-violet-900 dark:text-violet-200" :
              order.kitchenStatus === "cooking"   ? "bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200" :
            ...
            <div className="rounded-lg border bg-background/50 overflow-hidden">
              {order.items.map((item, idx) => (
                <div
                  ...
                  data-testid={`bar-order-item-${item.id}`}
                >
                  <span className="font-medium flex items-center gap-2 min-w-0">
                    <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-violet-500/10 text-violet-600 dark:text-violet-400 text-xs font-bold shrink-0">
                      {item.quantity}
                    </span>
              ...
              <div className="flex items-center justify-between px-2 sm:px-3 py-2 bg-muted/30 text-sm font-semibold border-t">
                ...
                <span className="text-violet-500">{(order.totalCents / 100).toFixed(2)} ₼</span>
              </div>
            </div>
          ) : (
          ...
          <div className="flex gap-2 flex-wrap pt-1">
            ...
            {order.kitchenStatus === "cooking" && (
              <Button
                ...
                disabled={updateStatus.isPending}
                data-testid={`button-bar-ready-${order.id}`}
              >
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <div className="p-1.5 sm:p-2 bg-violet-600 rounded-lg shrink-0">
              ...
              <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                ...
                <Button
                  ...
                  data-testid="button-bar-refresh"
                >
            ...
            {isMobile && (
              <div className="flex gap-2 mt-3" data-testid="tabs-bar-mobile">
                {mobileTabs.map(tab => (
                  <button
                    key={tab.key}
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="space-y-4">
                  {mixing.map(o => <OrderCard key={o.id} order={o} />)}
                  {mixing.length === 0 && <div className="flex items-center justify-center py-12 text-slate-600 text-sm">—</div>}
                </div>
                <div className="space-y-4">
